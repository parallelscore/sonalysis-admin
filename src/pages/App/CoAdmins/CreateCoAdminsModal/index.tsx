...
const CreateCoAdmin = ({ showInvitationModal, setShowModal, name }) => {
    ...
    function createCoAdmin(file) {
        ...
        cloudinaryPostCall(endPoint.cloudinaryPost, cloudinaryData).then(
            (res) => {
                if (res?.status === 200) {
                    ...
                    postCall(endPoint.createCoAdmin, coAdminData).then(
                        (res) => {
                            setIsLoading(false);
                            if (res?.status === 200) {
                                swal(
    ...
    const checkCompleted =
        coAdminData.email && coAdminData.fullName && fileLogo ? true : false;
                    ...
                    <div className='user_icon_container'>
                        <div className='icon_container'>
                            <label htmlFor='photo' className='user-photo'>
                                <img
                                    src={fileLogo ? fileLogo : UserIcon}
                                    alt='logo'
                                />
                            </label>
                            <input
