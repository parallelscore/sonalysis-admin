...
const Analytics = () => {
  ...
  const updateClub = (clubId, data)=>{
    setIsLoading(true)
    putCall(endPoint.clubUpdate(clubId), data)
    .then((response) => {
      setIsLoading(false)
      if (response.status === 200) {
        swal("club updated successfully", {
          icon: "success",
        });
  ...
  const  getAllClubs = (status)=>{
    setIsLoading(true)
    getCall(endPoint.getAllClub(status))
    .then((response) => {
      setIsLoading(false)
      if (response.status === 200) {
        setClubData(response.data.data)
      ...
      <div className="all-files">
        ...
        <div className="video-tab">
          ...
          {isLoading ? <div className="d-flex align-items-center justify-content-center mt-5"><LoopingRhombusesSpinner color="#811aff" /></div> :
            <div>
              {
                clubData?.data && clubData?.data.slice(0,5)?.map((item, id) => (
                 ...
                 <div   className="table-row d-flex align-items-center p-3 mt-4">
                    ...
                    <div className={`col-2 ${item.status!=="pending" ? "success" : "pending"} status`}>
                      {item.status}
                    </div>
                    <div className="col-2">
                    ...
                    <div className="btn-group">
                        ...
                        <ul className="dropdown-menu">
                          <Link  to="/app/clubs/details">

                          <li className="dropdown-item">
                              Edit
                          </li>
                          </Link>
                          <li className="dropdown-item" onClick={()=>updateClub(item._id, {"status":"approved"})}>
                              Approve
                          </li>
                          <li className="dropdown-item" onClick={()=>updateClub(item._id, {"status":"rejected"})}>
